...
{$ENDIF}
  ...
  screen := SDL_CreateRGBSurface(SDL_HWSURFACE, CENTER_X * 2, CENTER_Y * 2, 32, 0, 0, 0, 0);
  prescreen := SDL_CreateRGBSurface(screen.flags, CENTER_X * 2, CENTER_Y * 2, 32, 0, 0, 0, 0);
  ImgScence := SDL_CreateRGBSurface(screen.flags, 2304, 1402, 32, 0, 0, 0, 0);
  ImgScenceBack := SDL_CreateRGBSurface(screen.flags, 2304, 1402, 32, 0, 0, 0, 0);
  ImgBField := SDL_CreateRGBSurface(screen.flags, 2304, 1402, 32, 0, 0, 0, 0);
  ImgBBuild := SDL_CreateRGBSurface(screen.flags, 2304, 1402, 32, 0, 0, 0, 0);
  ...
  case MODVersion of
    ...
    62:
      begin
        ...
        writeln(sizeof(TItem));
        writeln(sizeof(TScence));
        writeln(sizeof(TMagic));
        writeln(sizeof(TShop));}
      end;
  end;
...
begin
  ...
  begin
    ...
    drawtextwithrect(@scencename[1], 320 - length(PChar(@rscence[snum].Name)) * 5 + 7, 100,
...
begin
  ...
  for i := 0 to min(max, length(Menustring) - 1) do
    ...
    begin
      ...
      if p = 1 then
        drawengshadowtext(@menuengstring[i][1], x + 93, y + 2 + 22 * i, colcolor($66), colcolor($64));
    end
    else
    begin
      drawshadowtext(@menustring[i][1], x - 17, y + 2 + 22 * i, colcolor($7), colcolor($5));
      if p = 1 then
        drawengshadowtext(@menuengstring[i][1], x + 93, y + 2 + 22 * i, colcolor($7), colcolor($5));
    end;
...
begin
  ...
  for i := 0 to max do
    ...
    begin
      ...
      drawshadowtext(@word[i][1], 10, 32 + 22 * i, colcolor($66), colcolor($64));
    end
    else
    begin
      //drawtext(screen, @word[i][1], 11, 32 + 22 * i, colcolor($7));
      drawshadowtext(@word[i][1], 10, 32 + 22 * i, colcolor($7), colcolor($5));
    end;
  SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        drawshadowtext(@words3[i][1], 97 + i1 mod 3 * 130, ((len2 + 2) div 3 + i1 div 3) *
          20 + 346, colcolor($50), colcolor($4E));
        drawshadowtext(@str[1], 147 + i1 mod 3 * 130, ((len2 + 2) div 3 + i1 div 3) * 20 +
...
begin
  ...
  begin
    ...
    menustring[1] := (' 繼續');
    str := (' 是否自宫？');
    drawtextwithrect(@str[1], 80, 30, 105, colcolor(7), colcolor(5));
...
begin
  ...
  DrawRectangle(x, y, 525, 315, 0, colcolor(255), 50);
  ...
  drawshadowtext(@strs[19, 1], x + 340, y + 5, colcolor($23), colcolor($21));
  ...
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 0, colcolor($7), colcolor($5));
  str := format('%4d', [Rrole[rnum].Defence + adddef]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 1, colcolor($7), colcolor($5));
  str := format('%4d', [Rrole[rnum].Speed + addspeed]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 2, colcolor($7), colcolor($5));

  //其他属性
  str := format('%4d', [Rrole[rnum].Medcine]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 3, colcolor($7), colcolor($5));

  str := format('%4d', [Rrole[rnum].UsePoi]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 4, colcolor($7), colcolor($5));

  str := format('%4d', [Rrole[rnum].MedPoi]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 5, colcolor($7), colcolor($5));

  str := format('%4d', [Rrole[rnum].Fist]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 6, colcolor($7), colcolor($5));

  str := format('%4d', [Rrole[rnum].Sword]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 7, colcolor($7), colcolor($5));

  str := format('%4d', [Rrole[rnum].Knife]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 8, colcolor($7), colcolor($5));

  str := format('%4d', [Rrole[rnum].Unusual]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 9, colcolor($7), colcolor($5));

  str := format('%4d', [Rrole[rnum].HidWeapon]);
  drawengshadowtext(@str[1], x + 280, y + 5 + 21 * 10, colcolor($7), colcolor($5));
  ...
  begin
    ...
    if magicnum > 0 then
    begin
      drawbig5shadowtext(@Rmagic[magicnum].Name, x + 340, y + 26 + 21 * i, colcolor($7), colcolor($5));
      str := format('%3d', [Rrole[rnum].MagLevel[i] div 100 + 1]);
      drawengshadowtext(@str[1], x + 480, y + 26 + 21 * i, colcolor($66), colcolor($64));
    end;
  end;
  ...
  drawshadowtext(@strs[18, 1], x + 340, y + 240, colcolor($23), colcolor($21));
  if Rrole[rnum].Equip[0] >= 0 then
  ...
  begin
    ...
    drawbig5shadowtext(@Ritem[Rrole[rnum].PracticeBook].Name, x + 350, y + 261, colcolor($7), colcolor($5));
    str := format('%5d/%5d', [uint16(Rrole[rnum].ExpForBook), needexp]);
    if mlevel = 10 then
      str := format('%5d/=', [uint16(Rrole[rnum].ExpForBook)]);
    drawengshadowtext(@str[1], x + 380, y + 282, colcolor($66), colcolor($63));
  end;
...
begin
  ...
  begin
    str := ' 場景內不可離隊！';
    drawtextwithrect(@str[1], 80, 30, 172, colcolor($23), colcolor($21));
    waitanykey;
  end;
  redraw;
  SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
...
begin
  ...
  begin
    ...
    begin
      DrawTextWithRect(@str[1], 100, 200, 150, $FFFFFFFF, $FFFFFFFF);
      waitanykey;
    end;
  end;
  if menu <> 1 then
...
begin
  ...
  begin
    redraw;
    DrawRectangle(115, 98, 155, 76, 0, colcolor(255), 30);
...
begin
  ...
  begin
    redraw;
    DrawRectangle(115, 98, 155, 51, 0, colcolor(255), 30);
    word := (' 減少中毒');
    drawshadowtext(@word[1], 100, 125, colcolor($7), colcolor($5));
    drawbig5shadowtext(@rrole[role2].Name, 100, 100, colcolor($23), colcolor($21));
...
begin
  ...
  begin
    l := p div 2 + p mod 2;
    twoline := 1;
    Drawrectangle(20, 100, 400, 22 * l + 25, 0, colcolor($FF), 25);
  end;
  if twoline = 0 then
    x := 83
  else
    x := 3;
  drawbig5shadowtext(@rrole[rnum].Data[4], x, 102, colcolor($23), colcolor($21));
  str := (' 未增加屬性');
  if p = 0 then
