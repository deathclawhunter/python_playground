...
procedure DrawBFieldWithEft2(Epicnum, beginpic, endpic: integer); overload;
procedure DrawBFieldWithEft2(Epicnum, beginpic, endpic, curlevel: integer); overload;
...
//procedure SetPngTileBlock(PNGTile: TPNGTile; num, px, py, depth: integer; BlockImageW: pchar; width, height, size: integer);
...
//当BlockImageW不为空时, 将该值写入BlockImageW, 如果该值超出范围(0~128), 会根据图片的y坐标计算
  ...
  begin
    ...
    begin
      if MPNGIndex[num].UseGRP = 0 then
        DrawPNGTile(MPNGIndex[num], sdl_getticks div 200, screen, px, py)
      else
        NeedGRP := 1;
    end;
  ...
  begin
    ...
    if PNG_Tile = 1 then
      DrawPngTile(SPNGIndex[num], 0, screen, px, py, shadow, alpha, mixColor, mixAlpha,
        depth, @BlockImg[0], 2304, 1402, sizeof(integer), BlockScreen.x, BlockScreen.y)
    else
    begin
      Area.x := 0;
      Area.y := 0;
      ...
      DrawRLE8Pic(@ACol[0], num, px, py, @SIdx[0], @SPic[0], @Area, nil, 0, 0, 0, shadow, alpha,
        @BlockImg[0], @BlockScreen, 2304, 1402, sizeof(integer), depth, mixColor, mixAlpha);
    end;
  end;
...
begin
  ...
  if (num >= 0) and (num < SPicAmount) then
    ...
    begin
      ...
      if needBlock <> 0 then
      begin
        SetPNGTileBlock(SPNGIndex[num], px, py, depth, pBlock, 2304, 1402, sizeof(integer));
      end;
    end
    else
    begin
      ...
      begin
        ...
        DrawRLE8Pic(@ACol[0], num, px, py, @SIdx[0], @SPic[0], @Area, pImg, 2304, 1402, sizeof(UInt32), 0);
    end;
end;
...
procedure DrawHeadPic(num, px, py: integer); overload;
begin
  DrawHeadPic(num, px, py, 0, 0, 128, 0, 0);
end;
...
procedure DrawBPic(num, px, py, shadow: integer); overload;
begin
  DrawBPic(num, px, py, shadow, 0, 128, 0, 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      if needBlock <> 0 then
      begin
        SetPNGTileBlock(BPNGIndex[num], px, py, depth, @BlockImg[0], 2304, 1402, sizeof(integer));
        pImg := ImgBBuild;
      end
      else
        pImg := ImgBfield;
    ...
    begin
      ...
      if needBlock <> 0 then
        DrawRLE8Pic(@ACol[0], num, px, py, @WIdx[0], @WPic[0], @Area, ImgBBuild, 2304, 1402,
          sizeof(UInt32), 0, 0, @BlockImg[0], nil, 0, 0, 0, depth, 0, 0)
      else
        DrawRLE8Pic(@ACol[0], num, px, py, @WIdx[0], @WPic[0], @Area, ImgBfield, 2304, 1402, sizeof(UInt32), 0);
    end;
  end;
end;
...
procedure DrawEPic(num, px, py: integer); overload;
begin
  DrawEPic(num, px, py, 0, 0, 128, 0, 0);
...
procedure DrawFPic(num, px, py, index: integer); overload;
begin
  DrawFPic(num, px, py, 0, 0, 128, 0, 0, index);
    ...
    0:
      begin
        ...
        DrawRLE8Pic(@ACol[0], num, px, py, @FIdx[0], @FPic[0], @Area, nil, 0, 0, 0, shadow, alpha,
          @BlockImg[0], @BlockScreen, screen.w, screen.h, sizeof(integer), depth, mixColor, mixAlpha);
      end;
  end;
end;
...
begin
  ...
  for sum := -29 to 41 do
    ...
    begin
      ...
      if (i1 >= 0) and (i1 < 480) and (i2 >= 0) and (i2 < 480) then
      begin
        if (sum >= -27) and (sum <= 28) and (i >= -9) and (i <= 9) and (BIG_PNG_TILE = 0) then
        begin
          DrawMPic(earth[i1, i2] div 2, pos.x, pos.y);
          if surface[i1, i2] > 0 then
            DrawMPic(surface[i1, i2] div 2, pos.x, pos.y);
...
procedure InitialScence(Visible: integer); overload;
var
  i1, i2, x, y, x1, y1, w, h, sumi, num, i, depth: integer;
  pos: TPosition;
  temp: integer;
  dest: TSDL_Rect;
begin
  ...
  for i1 := 0 to 63 do
    ...
    begin
      ...
      //InitialSPic(SData[CurScence, 1, i1, i2] div 2, x, y, x1, y1, w, h, 1);
  ...
  //writeln(BlockImg[1000, 1000],BlockImg2[1000, 1000]);
  if (Visible = 2) and (where = 1) and (x1 >= 0) and (x1 < 2304 - w) and (y1 >= 0) and (y1 < 1402 - h) then
  begin
    Move(BlockImg2[x1, 0], BlockImg[x1, 0], w * 1402 * 4);
    dest.x := x1;
    dest.y := y1;
    dest.w := w;
    dest.h := h;
...
begin
  ...
  for i1 := 0 to 63 do
    ...
    begin
      ...
      if (i1 < 0) or (i2 < 0) or (i1 > 63) or (i2 > 63) then
      begin
        InitialBPic(0, x, y, 0, 128);
      end
      else
      begin
        depth := 128 * min(i1, i2) + abs(i1 - i2);
        InitialBPic(bfield[0, i1, i2] div 2, x, y, 0, 128);
        if (bfield[1, i1, i2] > 0) then
        begin
          InitialBPic(bfield[1, i1, i2] div 2, x, y, 1, depth);
          //InitialBPic(bfield[1, i1, i2] div 2, x, y, 1);
        end;
      end;
    end;
      ...
      begin
        if (Brole[bnum].Team <> Brole[Bfield[2, Bx, By]].Team) and (Bfield[4, i1, i2] > 0) then
          DrawBPic(Rrole[Brole[bnum].rnum].HeadNum * 4 + Brole[bnum].Face + BEGIN_BATTLE_ROLE_PIC,
            pos.x, pos.y, 0, 75, i1 + i2, $FFFFFFFF, 20)
        else
          DrawBPic(Rrole[Brole[bnum].rnum].HeadNum * 4 + Brole[bnum].Face + BEGIN_BATTLE_ROLE_PIC,
  ...
  for i1 := 0 to 63 do
    ...
    begin
      ...
      if Bfield[4, i1, i2] > 0 then
        DrawEPic(Epicnum, pos.x, pos.y, 0, 25, 128, 0, 0);
    end;
  DrawProgress;
  //BFieldDrawn := 1;
  {if (SDL_MustLock(screen)) then
  ...
  for i1 := 0 to 63 do
    ...
    begin
      ...
      if (Bfield[4, i1, i2] > 0) and (Epicnum - Bfield[4, i1, i2] + 1 >= beginpic) and
        ...
        DrawEPic(Epicnum - Bfield[4, i1, i2] + 1, pos.x, pos.y, 0, 25, 128, 0, 0);
    end;
  DrawProgress;
  ...
  for i1 := 0 to 63 do
    ...
    begin
      ...
      begin
        ...
        begin
          DrawEPic(k, pos.x, pos.y, 0, 25, 128, 0, 0);
...
procedure DrawPngTile(PNGIndex: TPNGIndex; FrameNum: integer; scr: PSDL_Surface; px, py: integer; shadow, alpha: integer; MixColor: Uint32; MixAlpha: integer); overload;
begin
  DrawPngTile(PNGIndex, FrameNum, scr, px, py, shadow, alpha, MixColor, MixAlpha, 128, nil, 0, 0, 0, 0, 0);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (MixAlpha > 0) then
            begin
              //首先将资源画至纯色背底, 再在其上以半透明画纯色
              //两种相同颜色的混合值是一样的
              tempscrfront := SDL_DisplayFormat(CurSurface);
              tempscrback := SDL_DisplayFormat(CurSurface);
              SDL_FillRect(tempscrfront, nil, MixColor);
              ...
              SDL_BlitSurface(tempscrfront, nil, tempscrback, nil);
